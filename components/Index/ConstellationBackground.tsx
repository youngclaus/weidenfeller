...
const ConstellationBackground: React.FC<ConstellationBackgroundProps> = ({ interactiveVisible = true }) => {
  ...
  useEffect(() => {
    ...
    function updateParticles() {
      ...
      constellationOpacityRef.current = Math.abs(opacityDelta) < 0.002
        ...
        : constellationOpacityRef.current + opacityDelta * 0.16;
  ...
  return <Background><Canvas ref={canvasRef} aria-hidden="true" /><Vignette aria-hidden="true" /><ConstellationLayer $visible={interactiveVisible} aria-label="Interactive constellations">{namedConstellations.map((item) => {
    ...
    return <ConstellationHotspot key={item.id} type="button" style={style} aria-label={`Reveal constellation: ${item.title}`} aria-pressed={selected} $visible={interactiveVisible} onPointerEnter={() => setActive(item.id)} onPointerLeave={() => setActive(null)} onFocus={() => setActive(item.id)} onBlur={() => setActive(null)} onClick={() => setActive(activeRef.current === item.id ? null : item.id)}><ConstellationLabel $visible={selected}>{item.title}</ConstellationLabel></ConstellationHotspot>;
...
const ConstellationLayer = styled.div<{ $visible: boolean }>`position:absolute;inset:0;z-index:2;opacity:${({ $visible }) => $visible ? 1 : 0};pointer-events:none;transition:opacity 180ms ease;@media(prefers-reduced-motion:reduce){transition-duration:1ms;}`;
