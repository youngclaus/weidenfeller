...
const ConstellationBackground: React.FC = () => {
  ...
  useEffect(() => {
    ...
    function drawConstellations() {
      ...
      namedConstellations.forEach((item, constellationIndex) => {
        ...
        context.globalAlpha = selected ? 0.94 : 0.38;
        ...
        points.forEach((point, pointIndex) => {
          ...
          context.globalAlpha = selected ? 1 : 0.74;
          context.shadowColor = selected ? theme.glow : theme.c3;
    ...
    function draw() {
      ...
      particles.forEach((particle) => {
        ...
        context.shadowBlur = boost > 0.2 ? 8 : 3;
        context.fill();
      });
      drawConstellations();
  ...
  return <Background><Canvas ref={canvasRef} aria-hidden="true" /><Vignette aria-hidden="true" /><ConstellationLayer aria-label="Interactive constellations">{namedConstellations.map((item) => {
    const selected = active === item.id;
    const style: ConstellationStyle = { '--desktop-x': `${item.x}%`, '--desktop-y': `${item.y}%`, '--mobile-x': `${item.mobileX}%`, '--mobile-y': `${item.mobileY}%` };
    return <ConstellationHotspot key={item.id} type="button" style={style} aria-label={`Reveal constellation: ${item.title}`} aria-pressed={selected} onPointerEnter={() => setActive(item.id)} onPointerLeave={() => setActive(null)} onFocus={() => setActive(item.id)} onBlur={() => setActive(null)} onClick={() => setActive(activeRef.current === item.id ? null : item.id)}><ConstellationLabel $visible={selected}>{item.title}</ConstellationLabel></ConstellationHotspot>;
